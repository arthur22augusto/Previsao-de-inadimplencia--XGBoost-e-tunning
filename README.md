...
### O seguinte fluxgrama detalha a metodologia e divisão da análise

```mermaid
graph TD;
    A["Início: Coleta de Dados<br/>(Bases: Cadastral, Info, Pagamentos)"] --> B["Etapa 1: Análise Exploratória<br/>(notebook 1_analise_exploratoria)<br/>- Limpeza Inicial<br/>- Entendimento dos Dados (EDA)"];
    B --> C["Etapa 2: Processamento e Engenharia<br/>(notebook 2_processamento_dos_dados)<br/>- Tratamento de Nulos e Encoding<br/>- Criação de Variáveis de Histórico"];
    C --> D["Divisão Estratificada dos Dados<br/>(Treino 80% / Teste 20%)"];
    D --> E["Etapa 3: Modelagem Base<br/>(notebook 3_ajuste_modelo)<br/>- Treino do XGBoost com `scale_pos_weight`<br/>- Avaliação de Performance Inicial"];
    E --> F["Etapa 4: Otimização do Modelo<br/>(notebook 4_melhorando_o_ajuste)<br/>- Ajuste de Hiperparâmetros com Optuna<br/>- Validação Cruzada para Maximizar ROC AUC"];
    F --> G["Seleção do Modelo Final<br/>(Modelo Otimizado com Melhor Performance)"];
    G --> H["Etapa 5: Geração das Previsões<br/>(notebook 5_previsao)<br/>- Aplicar mesmas transformações na base de teste<br/>- Gerar probabilidades com o modelo final"];
    H --> I["Resultado Final<br/>(Arquivo submissao_case.csv)"];
```
